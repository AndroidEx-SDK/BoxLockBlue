...
//            }
        ...
        public RemoteViews getViewAt(int position) {
            // TODO Auto-generated method stub
            RemoteViews rv = new RemoteViews(mContext.getPackageName(), com.androidex.capbox.R.layout.list_item_provider_widget);
            ...
            rv.setTextViewText(com.androidex.capbox.R.id.tv_name, device.getDeviceName());
            rv.setTextViewText(com.androidex.capbox.R.id.tv_address, device.getMac());

            rv.setOnClickFillInIntent(com.androidex.capbox.R.id.rl_device, getPendingIntentStartActivity(device.getDeviceName(), device.getMac(), device.getUuid(), position));
            rv.setOnClickFillInIntent(com.androidex.capbox.R.id.iv_connect, getIntent(CLICK_BLE_CONNECTED, position, device.getMac()));
            rv.setOnClickFillInIntent(com.androidex.capbox.R.id.iv_lock, getIntent(CLICK_LOCK_OPEN, position, device.getMac()));

            if (MyBleService.getInstance().getConnectDevice(device.getMac()) == null) {
                rv.setImageViewResource(com.androidex.capbox.R.id.iv_connect, com.androidex.capbox.R.mipmap.starts_connect2);
            } else {
                rv.setImageViewResource(com.androidex.capbox.R.id.iv_connect, com.androidex.capbox.R.mipmap.starts_disconnect2);
            }
            return rv;
        }
                ...
                if (model != null) {
                    switch (model.code) {
                        case Constants.API.API_OK:
                            ...
                            AppWidgetManager.getInstance(context).notifyAppWidgetViewDataChanged(mAppWidgetId, com.androidex.capbox.R.id.myListView);
                            RLog.d("刷新列表");
                            break;
