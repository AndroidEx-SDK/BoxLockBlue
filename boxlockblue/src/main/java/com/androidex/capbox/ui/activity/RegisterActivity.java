...
//                    initv(username);
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        tv_getCaptcha.setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View view, MotionEvent motionEvent) {
                if(motionEvent.getAction() == MotionEvent.ACTION_DOWN){
                    //更改为按下时的背景图片
                    tv_getCaptcha.setTextColor(Color.parseColor("#2495F7"));
                    Drawable img_right;
                    Resources res = getResources();
                    img_right = res.getDrawable(R.mipmap.authentication1);
                    //调用setCompoundDrawables时，必须调用Drawable.setBounds()方法,否则图片不显示
                    img_right.setBounds(0, 0, img_right.getMinimumWidth(), img_right.getMinimumHeight());
                    tv_getCaptcha.setCompoundDrawables(null, null, img_right, null);
                }else if(motionEvent.getAction() == MotionEvent.ACTION_UP){
                    //改为抬起时的图片
                    tv_getCaptcha.setTextColor(Color.parseColor("#BFBFBF"));
                    Drawable img_right;
                    Resources res = getResources();
                    img_right = res.getDrawable(R.mipmap.authentication2);
                    //调用setCompoundDrawables时，必须调用Drawable.setBounds()方法,否则图片不显示
                    img_right.setBounds(0, 0, img_right.getMinimumWidth(), img_right.getMinimumHeight());
                    tv_getCaptcha.setCompoundDrawables(null, null, img_right, null);
                }
...
//				mProgressBar.setProgress((int) (100 * percent));
    ...
    public void setRegisterBtnEnable() {
        ...
        if (TextUtils.isEmpty(phone)
            ...
            Drawable drawable;
            Resources res = getResources();
            drawable = res.getDrawable(R.mipmap.bt2);
            tv_register.setBackground(drawable);
